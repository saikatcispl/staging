...
<div class="portlet box green">
    <div class="portlet-title">
        ...
        <div class="pull-right">
            <a href="{{ URL::to('admin/addProduct') }}" class="btn btn-primary" title="Add Product to sync from LimeLight CRM.">+ Add Product</a>
        </div>
    </div>
...
<script>
    $(function () {
        $('#product-table').DataTable({
            serverSide: true,
            processing: true,
            ajax: '<?= SITEURL('admin/manageProducts-data') ?>',
            columns: [
                {data: 'id'},
                {data: 'll_product_id'},
                {data: 'product_name'},
    ...
    $(document).on('click', '.manuallySyncProductFromLL', function (e) {
        ...
        var _this = $(this);
        var request = $.ajax({
            url: "{{ URL::to('admin/manuallySyncProductFromLL') }}" + '?ll_product_id=' + _this.data('ll_product_id'),
            method: "GET",
            dataType: "json"
        });
